...
account_data = {
    "account_id": str(flagged["account_id"]),
    "segment": str(flagged["segment"]),
    "outstanding_balance": f"${flagged['outstanding_balance']:,.0f}",
    "z_score": float(flagged["z_score"]),
    "composite_severity": float(flagged["composite_severity"]),
    "rating": str(flagged["rating"]),
    "months_on_book": int(flagged["months_on_book"])
}
